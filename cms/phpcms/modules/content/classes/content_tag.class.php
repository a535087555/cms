...
class content_tag {
	...
	//导航条
	 public function navlist($data){
	     pc_base::load_model('ily_nav_model');
	     $_ily_nav = new ily_nav_model();
	     $allnav = $_ily_nav->select();
	     
	    return $allnav;
	 }
	
   //超链接的内容
	 public function ilylinks(){
	 
	      pc_base::load_model('ily_links_model');
	      ...
	      $_links=new ily_links_model();
	      
	    /*   foreach ($_catid as $_key => $_val) {
	          $_linksinfo[$_key] = $_links->select(array(
	              'catid' => "$_val"
	          ));
	          
	          SELECT * FROM v9_ilylinks AS a JOIN v9_ilylinks_data AS b WHERE a.id=b.id
	      } */
	      foreach ($_catid as $_key => $_val) {
	          $_linksinfo[$_key] = $_links->query("SELECT * FROM v9_ilylinks AS a JOIN v9_ilylinks_data AS b WHERE a.id=b.id and a.catid='.$_val.'")->fetch_all();
	      }
	      
	      
	      $_arr['categoryinfo']=$_categoryinfo;
	      
	     
// 	      foreach ($_linksinfo as $k=>$v){
// 	          foreach ($v as  $k1=>$v1){	              
// 	                   $_newarr[]=$v1['title'];	              
// 	          }
// 	      }

	      $_arr['linksinfo']=$_linksinfo;  
	      return $_arr;
	    }
